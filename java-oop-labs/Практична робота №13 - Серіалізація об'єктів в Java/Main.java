...
public class Main {
    ...
    private static void askRestoreOnStartup() {
        System.out.print("Відновити базу користувачів з файлу? (y/n): ");
    ...
    private static void askSaveOnExit() {
        System.out.print("Зберегти базу користувачів у файл перед виходом? (y/n): ");
        String answer = scanner.nextLine().trim();
        if (answer.equalsIgnoreCase("y")) {
            System.out.print("Шлях до файлу: ");
            String path = scanner.nextLine().trim();
    ...
    private static void doGetFiltered() {
        ...
        System.out.println("  3 — зареєстровані більше хвилини тому");
        System.out.print("Ваш вибір: ");
        String sub = scanner.nextLine().trim();
    ...
    private static void doSave() {
        System.out.print("Шлях до файлу для збереження: ");
        String path = scanner.nextLine().trim();
        registry.saveToFile(path);
    }

    private static void doLoad() {
        System.out.print("Шлях до файлу для завантаження: ");
        String path = scanner.nextLine().trim();
        registry.loadFromFile(path);
    }
}
